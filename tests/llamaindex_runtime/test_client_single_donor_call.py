...
class TestPageIndexClientSingleDonorCall:
    ...
    def test_client_markdown_path_calls_md_to_tree_only_once(
        ...
        with patch("pageindex.page_index_md.md_to_tree", side_effect=mock_md_to_tree):
            ...
            with patch(
                ...
                client.index(str(test_md), mode="md", write_to_registry=False)
    ...
    def test_client_markdown_path_success_case_no_double_call(
        ...
        """MIGRATED: Test that successful markdown path doesn't call adapter.index_tree().

        Verify: When md_to_tree succeeds, client should:
        1. Call md_to_tree once
        2. Return workspace doc_id (no registry.write_tree - consumer-only)
        """
        monkeypatch.setenv("OPENAI_API_KEY", "test-key-for-tdd")
        ...
        assert adapter_index_tree_call_count == 0, (
            ...
            "client directly stores structure in workspace"
        )

        # CONSUMER-ONLY: registry.write_tree should NOT be called
        assert not registry.write_tree.called, (
            "registry.write_tree was called - PageIndex is consumer-only"
        )
...
class TestAdapterLayerUnifiedSeamNotModified:
    ...
    def test_adapter_layer_md_to_tree_unified_seam_preserved(
        ...
        """MIGRATED: index_tree is FORBIDDEN - verify RuntimeError is raised.

        Phase 15: index_tree() is FORBIDDEN and must raise RuntimeError.
        The unified seam is still used by _call_pageindex_md_to_tree for non-canonical use.
        """
        monkeypatch.setenv("OPENAI_API_KEY", "test-key-for-tdd")
