"""Shared AST proof utilities for the Task #88 global acceptance proof.

Non-collected (underscore prefix) support for
``test_phase15_e2a_task88_global_acceptance_proof``: pure ``ast`` shape
verifiers and parsed-only fixture builders for the selected live
acceptance test and the sanitized-env builder.
...
collection and out of the counted ``test_phase15_e2a_task88_*.py``
glob). It only parses source text and inspects ASTs; fixtures built here
are parsed only, never executed.
"""
...
# The 12 C14 observation fields of the approved composed proof's live
# component (LateDmlCandidateObservations), each asserted exactly once.
REQUIRED_OBSERVATION_FIELDS = frozenset(
    {
        "outcome_rolled_back_failure",
        "failure_maps_empty",
...
LIVE_TEST_NAME = "test_task88_authorized_live_acceptance"
METADATA_TEST_NAME = "test_task88_global_acceptance_metadata"
HELPER_IMPL_FUNCTION = "_run_late_dml_candidate_impl"
CANDIDATE_SELECTOR_TEST = "test_phase15_e2a_late_dml_candidate_live"
CANDIDATE_SELECTOR_MODULE = "phase15_e2a_late_dml_candidate"
...
def _verify_sanitized_env_structure(function: ast.FunctionDef) -> None:
    ...
    form: the base must be the plain name ``os`` and the attribute must
    be ``getenv`` (``alias = os`` followed by ``alias.environ`` or any
    other aliased or container attribute read is rejected). The
    collected literal keys must equal the allowlist exactly and stay
    disjoint from the sensitive variable names. Indirect broad
    environment access (``getattr(os, "getenv")(...)``, alias imports,
    subscript indirection, container reads without a call) is rejected
    structurally, not by substring matching.
    """
...
def _verify_session_keyword_value(keyword: ast.keyword) -> None:
    ...
    if keyword.arg == "container_name":
        ...
        assert value.func.id == "_validate_container_name", (
        ...
        assert len(value.args) == 1, (
            "container_name must receive exactly one argument"
        )
        inner = value.args[0]
        ...
        assert inner.func.id == "_generate_safe_container_name", (
            "the container_name argument must be _generate_safe_container_name()"
        )
        assert not inner.args and not inner.keywords, (
        ...
        assert value.func.id == "_select_ephemeral_port", (
            "port must be _select_ephemeral_port()"
        )
        ...
        assert value.func.id == "_generate_test_password", (
            "password must be _generate_test_password()"
        )
        assert not value.args and not value.keywords, (
...
def _verify_live_acceptance_body(function: ast.FunctionDef) -> None:
    ...
    - exactly one ``with contextlib.ExitStack() as stack:`` block that
      ...
      EXACTLY the approved safe value expression
      (``_validate_container_name(_generate_safe_container_name())``,
      ``_select_ephemeral_port()``, ``_generate_test_password()``), enters
      it exactly once via ``stack.enter_context(session)``, and invokes
      ``_run_late_dml_candidate_impl(session)`` exactly once as a direct
    ...
    - exactly 12 success assertions, each of the form
      ``assert observations.<field>``, covering every required
      observation field exactly once.

    The live test must never nest the candidate pytest selector and must
    never call ``session.open_fresh_attested_connection`` itself. Any
    extra statement, assignment-wrapped or session-attribute call, altered
    argument, hardcoded or substituted session value, second helper call,
    missing authorization, or missing assertion fails here.
    """
    ...
    assert session_keywords == {"container_name", "port", "password"}, (
    ...
    assert len(enter_arguments) == 1 and isinstance(enter_arguments[0], ast.Name), (
    ...
    assert CANDIDATE_SELECTOR_TEST not in referenced_names, (
        "the live test must not nest the candidate pytest selector"
    )
    attribute_names = {
        node.attr for node in ast.walk(function) if isinstance(node, ast.Attribute)
    }
    assert "open_fresh_attested_connection" not in attribute_names, (
    ...
    assert len(failure_guards) == 1, "the failure raise must sit in exactly one if guard"
...
def _build_env_fixture_with_extra_read(extra_read: str, preamble: str = "") -> str:
    ...
    Every allowlist key is read directly so the fixture is a realistic
    candidate for a lenient verifier to accept; ``extra_read`` is the
    malicious line under test (for example
    ``getattr(os, "getenv")("DATABASE_URL")``) and ``preamble`` is an
    optional statement inserted before the allowlist loop (for example
    ``alias = os``). The fixture is only ever parsed, never executed,
...
def _live_acceptance_fixture(
    *,
    ...
    nested_candidate_selector: bool = False,
    hardcoded_session_value: bool = False,
) -> str:
    """Source of a live-test-shaped AST fixture with exactly one deviation.

    The baseline fixture mirrors the approved composed-proof live test
    body (auth first, one ExitStack/session, one direct positional helper
    call, one redacted failure raise, 12 success assertions). Each flag
    introduces exactly one malicious deviation: ``omit_auth`` removes the
    authorization check, ``session_open_assignment`` hides a session-open
    call in an assignment statement, ``second_helper_call`` duplicates
    the helper invocation, ``nested_candidate_selector`` nests the
    candidate pytest selector, and ``hardcoded_session_value`` substitutes
    a hardcoded string literal for the safe container-name expression.
    The fixture is only ever parsed, never executed, and never opens a
    session.
    """
    lines = ["def test_task88_authorized_live_acceptance():"]
    if not omit_auth:
    ...
    lines.append("        observations = _run_late_dml_candidate_impl(session)")
    if second_helper_call:
        lines.append("        observations = _run_late_dml_candidate_impl(session)")
    lines.append("    if not observations.success:")
    lines.append("        raise RuntimeError(")
    lines.append(
        '            f"Task #88 global acceptance cell failed: '
        "{observations.error_reason}\")"
    )
    if nested_candidate_selector:
        lines.append("    test_phase15_e2a_late_dml_candidate_live()")
