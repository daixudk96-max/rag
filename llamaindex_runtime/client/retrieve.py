"""PageIndex retrieve tool functions移植.

工具函数（PageIndex原版）：
- get_document：返回文档元数据
- get_document_structure：返回树结构（去掉text字段）
- get_page_content：提取页面内容（支持PDF和Markdown）

pages格式：
- PDF：物理页码（1-indexed），如'5-7', '3,8', '12'
- Markdown：行号对应node headers
"""
...
def get_document_structure(documents: Dict[str, Any], doc_id: str) -> str:
    ...
    Args:
        documents: PageIndex documents dict
        doc_id: Document ID

    Returns:
        JSON string with tree structure (no text fields)
    """
    doc_info = documents.get(doc_id)
    if not doc_info:
        return json.dumps({'error': f'Document {doc_id} not found'})
...
def get_page_content(documents: Dict[str, Any], doc_id: str, pages: str) -> str:
    ...
    Args:
        documents: PageIndex documents dict
        doc_id: Document ID
        pages: Page format string ('5-7', '3,8', or '12')

    Returns:
        JSON string list of {'page': int, 'content': str}

    Page semantics:
    - PDF: physical page numbers (1-indexed)
    - Markdown: line numbers corresponding to node headers
    """
    doc_info = documents.get(doc_id)
    if not doc_info:
        return json.dumps({'error': f'Document {doc_id} not found'})
