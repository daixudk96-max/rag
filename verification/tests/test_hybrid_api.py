...
def _seed_hybrid(db_connection, applied_schema):
    ...
    version_id = crud.create_version(db_connection, doc_id=doc_id, content_hash="hybrid-api-v1", normalization_contract_id=contract_id, version_no=1)
    spans = [
...
def test_hybrid_api(db_connection, applied_schema, monkeypatch) -> None:
    ...
    assert {"paths_used", "total_hits", "decision", "evidence"}.issubset(payload.keys())
